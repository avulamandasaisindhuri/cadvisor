...
function run_tests() {
  ...
  # Build in container matching target environment
  docker run --rm \
...
EOF
    ...
    /usr/local/bin/crictl pull public.ecr.aws/docker/library/busybox:1.32 || true && \
